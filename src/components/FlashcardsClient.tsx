  ...
  return (
    <div className="w-full text-center space-y-4 md:space-y-6">
      {lines.map((line, i) => {
        if (line.trim().startsWith('- ') || line.trim().startsWith('• ')) {
          return (
            <div key={i} className="flex gap-2 md:gap-4 items-start justify-center pl-2">
              ...
              <span className="text-gray-900 dark:text-gray-100 text-lg md:text-2xl lg:text-3xl text-left max-w-[90%] md:max-w-[85%] font-bold leading-snug">
        ...
        return (
          <p key={i} className="text-gray-900 dark:text-gray-100 leading-snug text-lg md:text-3xl lg:text-4xl font-bold max-w-prose mx-auto">
            {parseInline(line)}
          </p>
        )
      })}
...
function parseInline(text: string) {
  ...
  return parts.map((part, index) => {
    if (part.startsWith('`') && part.endsWith('`')) {
      ...
      return (
        <code key={index} className="bg-gray-100 dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 px-1.5 py-0.5 rounded-md font-mono text-base border border-gray-200 dark:border-slate-700 font-semibold">
          {code}
        </code>
      );
    }
...
6. Reste un expert pédagogue, clair et direct.`;
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-transparent text-gray-900 dark:text-gray-100 flex flex-col font-sans overflow-hidden">
      ...
      <main className="flex-1 w-full flex flex-col items-center justify-start overflow-y-auto pt-24 pb-12">
        ...
        <h1 className="text-3xl md:text-5xl font-black text-black dark:text-white tracking-tight text-center mb-8 mt-12">
        ...
        <div className="w-full max-w-4xl px-4 flex flex-col items-center justify-center relative gap-4">
          ...
          {loading ? (
            ...
            <div className="w-full max-w-lg text-center space-y-6 animate-fade-in py-10 bg-white dark:bg-slate-900 p-8 rounded-3xl shadow-lg border border-gray-100 dark:border-white/10">
              <div className="mx-auto w-24 h-24 bg-green-100 dark:bg-green-900/30 rounded-full flex items-center justify-center border border-green-200 dark:border-green-800">
                <Check size={48} className="text-green-600 dark:text-green-400" />
              </div>
              <h2 className="text-3xl font-black text-gray-900 dark:text-white">Session terminée !</h2>
              <p className="text-gray-500 dark:text-gray-400 font-medium">Bravo ! Vous avez terminé votre session de cartes mémo avec succès.</p>
            ...
            <div className="text-center space-y-6 py-10 bg-white dark:bg-slate-900 p-8 rounded-3xl shadow-lg border border-gray-100 dark:border-white/10 max-w-lg">
              <div className="mx-auto w-24 h-24 bg-indigo-50 dark:bg-indigo-900/30 rounded-full flex items-center justify-center border border-indigo-100 dark:border-indigo-800">
                <BrainCircuit size={48} className="text-indigo-600 dark:text-indigo-400" />
              </div>
              <h2 className="text-3xl md:text-5xl font-black text-gray-900 dark:text-white mb-4">Bibliothèque vide !</h2>
              <p className="text-gray-500 dark:text-gray-400 text-lg md:text-xl font-medium mb-10 max-w-md mx-auto">
                  ...
                  {!isExplaining ? (
                    ...
                    >
                      ...
                      >
                        <div
                          onClick={handleFlip}
                          className="absolute w-full h-full bg-white dark:bg-slate-900 border border-gray-200 dark:border-white/10 rounded-3xl p-6 md:p-12 flex flex-col items-center justify-center backface-hidden shadow-xl overflow-y-auto hide-scrollbar z-0 cursor-pointer hover:shadow-2xl transition-shadow"
                        >
                          ...
                          <div className="flex-1 flex flex-col items-center justify-center w-full mt-16">
                            <div className="text-xl md:text-3xl lg:text-4xl font-bold text-center leading-snug text-gray-900 dark:text-white max-w-prose px-2">
                        ...
                        {/* Back Face */}
                        <div className="absolute w-full h-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-3xl flex flex-col backface-hidden rotate-y-180 shadow-xl overflow-hidden z-0">

                          {/* Header (Top labels) */}
                          <div className="w-full flex-none flex flex-col items-center justify-center z-10 pt-6 pb-2 px-6">
                            <span className="text-lg md:text-xl font-black uppercase tracking-widest text-indigo-600 dark:text-indigo-400 bg-white/90 dark:bg-slate-800/90 px-8 py-2 rounded-full shadow-sm border border-indigo-100 dark:border-indigo-900/50 backdrop-blur-sm">
                              Réponse
                            </span>
                            <div className="mt-2 text-center w-full">
                              ...
                              <span className="text-xs md:text-sm font-medium text-gray-500 dark:text-gray-400 line-clamp-1 italic px-4">
                                &quot;{flashcards[currentCardIndex].front_content}&quot;
                              </span>
                            </div>
                          </div>
                          ...
                          <div className="hidden md:block absolute top-6 left-6">
                            <span className="text-xs font-semibold text-gray-400 dark:text-gray-500 bg-white dark:bg-slate-800 px-3 py-1.5 rounded-lg border border-gray-200 dark:border-gray-700">
                              {flashcards[currentCardIndex].category}
                            </span>
                          </div>
                          ...
                          <div className="flex-none w-full px-2 md:px-6 pb-4 pt-2 grid grid-cols-3 md:grid-cols-5 gap-2 md:gap-4 z-20 border-t border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-slate-900/50">
                            {/* 1. Inutile (Desktop only in footer) */}
                            <button
                              onClick={(e) => { e.stopPropagation(); handleMarkAsUseless(); }}
                              className="hidden md:flex pointer-events-auto group h-14 md:h-24 rounded-xl md:rounded-2xl border-2 bg-white dark:bg-slate-800 border-gray-100 dark:border-slate-700 text-gray-400 dark:text-gray-500 hover:bg-red-50 dark:hover:bg-red-900/20 hover:border-red-200 dark:hover:border-red-800 hover:text-red-500 shadow-sm transition-all flex-col items-center justify-center order-4 md:order-1 md:col-span-1"
                            ...
                            <button
                              ...
                              className={`pointer-events-auto group h-20 md:h-24 rounded-xl md:rounded-2xl border-2 transition-all flex flex-col items-center justify-center shadow-sm order-1 col-span-1 md:order-2 md:col-span-1
                               ${hoveredZone === Rating.Again
                                  ...
                                  : 'bg-white dark:bg-slate-800 border-red-50 dark:border-red-900/30 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/50 hover:border-red-200 dark:hover:border-red-800'}`}
                            >
                            ...
                            <button
                              ...
                              className={`pointer-events-auto group h-20 md:h-24 rounded-xl md:rounded-2xl border-2 transition-all flex flex-col items-center justify-center shadow-sm order-2 col-span-1 md:order-3 md:col-span-1
                               ${hoveredZone === Rating.Hard
                                  ...
                                  : 'bg-white dark:bg-slate-800 border-amber-50 dark:border-amber-900/30 text-amber-600 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-900/50 hover:border-amber-200 dark:hover:border-amber-800'}`}
                            >
                            ...
                            <button
                              ...
                              className={`pointer-events-auto group h-20 md:h-24 rounded-xl md:rounded-2xl border-2 transition-all flex flex-col items-center justify-center shadow-sm order-3 col-span-1 md:order-4 md:col-span-1
                               ${hoveredZone === Rating.Easy
                                  ...
                                  : 'bg-white dark:bg-slate-800 border-green-50 dark:border-green-900/30 text-green-600 dark:text-green-400 hover:bg-green-50 dark:hover:bg-green-900/50 hover:border-green-200 dark:hover:border-green-800'}`}
                            >
                            ...
                            <button
                              ...
                              className="hidden md:flex pointer-events-auto group h-14 md:h-24 rounded-xl md:rounded-2xl border-2 bg-white dark:bg-slate-800 border-indigo-50 dark:border-indigo-900/30 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/50 hover:border-indigo-200 dark:hover:border-indigo-800 shadow-sm transition-all flex-col items-center justify-center order-5 md:order-5 md:col-span-1"
                            >
