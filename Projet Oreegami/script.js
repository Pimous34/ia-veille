...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (coursePrepContainer) {
        // Use special renderer for Calendar Integration
        renderCoursePrepWithCalendar(coursePrepArticles, 'coursePrepArticles');
    }
    ...
    try {
        ...
        if (data && data.length > 0) {
            ...
            if (newsColumn) {
                ...
                mappedArticles.slice(0, 3).forEach(article => {
                    const card = document.createElement('div');
                    card.className = 'vignette-card';
                    card.style.cursor = 'pointer';
...
// Function to render Course Prep section with Calendar Integration
function renderCoursePrepWithCalendar(articles, containerId) {
    const container = document.getElementById(containerId);
    if (!container) return;

    // Calendar Configuration
    // INSTRUCTION FOR USER: Replace this ID with your public calendar ID
    // Example: "your_email@gmail.com" or a specific calendar ID found in settings
    const calendarId = "samueltessier973@gmail.com";

    // Calendar Embed URL construction (Agenda mode for 3-day like view)
    const calendarSrc = `https://calendar.google.com/calendar/embed?height=600&wkst=2&bgcolor=%23ffffff&ctz=Europe%2FParis&showTitle=0&showNav=1&showDate=1&showPrint=0&showTabs=1&showCalendars=0&showTz=0&mode=AGENDA&src=${encodeURIComponent(calendarId)}&color=%230B8043`;

    // 1. Calendar Component (Takes 2 slots in the grid)
    const calendarHTML = `
        <div class="calendar-wrapper">
             <iframe src="${calendarSrc}" width="800" height="600" frameborder="0" scrolling="no"></iframe>
        </div>
    `;

    // 2. Third Article (Takes 1 slot)
    // We display the 3rd article from the list (fallback to 1st if not enough)
    const articleToShow = articles[2] || articles[0];

    // Create html for the single article
    const articleHTML = articleToShow ? createArticleCard(articleToShow) : '';

    // Render both into the grid
    container.innerHTML = calendarHTML + articleHTML;

    // Log for debug
    console.log('Rendering Calendar + Article into', containerId);
}

// Export for external use
window.renderCoursePrepWithCalendar = renderCoursePrepWithCalendar;
